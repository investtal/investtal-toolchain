...
$Script:ModelMap = [ordered]@{
    'fable'        = @{ Id='cc/claude-fable-5';             Window='500000' }
    'opus'         = @{ Id='cc/claude-opus-4-8';            Window='500000' }
    ...
    'glm5'         = @{ Id='glm/glm-5.2';                   Window='500000' }
    'glmturbo'     = @{ Id='glm/glm-5-turbo';               Window='500000' }
    'deepseek'     = @{ Id='ds/deepseek-v4-pro';            Window='500000' }
    'dsflash'      = @{ Id='ds/deepseek-v4-flash';          Window='500000' }
...
function Update-9cc {
    ...
    if ($env:CC9_INSTALL_SOURCE) {
        ...
        $url = "https://raw.githubusercontent.com/investtal/investtal-toolchain/$latest/9cc/install.ps1"
        $env:CC9_VERSION = $latest
        $script = $null
        if (Get-Command gh -ErrorAction SilentlyContinue) {
