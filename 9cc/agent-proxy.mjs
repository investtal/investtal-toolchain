...
const UPSTREAM_URL = new URL(process.env.UPSTREAM_URL ?? "https://api.anthropic.com");
const UPSTREAM_PROTO = UPSTREAM_URL.protocol === "http:" ? http : https;
...
function handle(req, res) {
  ...
  req.on("end", () => {
    ...
    const upstream = UPSTREAM_PROTO.request(
      { hostname: UPSTREAM_URL.hostname, port: Number(UPSTREAM_PORT), path: reqPath, method: req.method, headers: forwardHeaders(req.headers, body) },
      (up) => {
